...
function login_bb() {
	global $_COOKIE,$wp_users_object;
	$users_data=$wp_users_object->get_user( $_COOKIE["user_email"], array( 'by' => 'email' ) );
	if (isset($users_data->user_login)) {
		$login= $users_data->user_login;
		$psw=$_COOKIE["user_password"];
		bb_logout();
		bb_set_auth_cookie( $users_data->ID,false );
		do_action('bb_user_login', (int)  $users_data->ID );
		bb_set_current_user($users_data->ID);
		
	}
}
if (isset($_COOKIE["user_email"])){
	if (strlen($_COOKIE["user_email"]) >0) {
		$user=bb_get_current_user();
		if (!($user)) {
		login_bb();
		reload_bb();
		}else if($user->data->user_email!=$_COOKIE["user_email"]) {
			bb_logout();
			login_bb();
			reload_bb();
		}
