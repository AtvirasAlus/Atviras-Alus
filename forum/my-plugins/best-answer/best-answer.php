...
$best_answer['automatic'] = false;	 	 //  set to false if you want to place manually in post.php template via   do_action('best-answer');
...
$best_answer['text'] = "Geras atsakymas";	//  text for link  (if any)
...
$best_answer['css'] = "
	...
	a.best_answer, a.best_answer:link {
		background-position:0px -64px;
	}
	a.best_answer:hover,
	a.best_answer_selected:hover {
		background-position:0px 0px;
	}	
	a.best_answer_selected {
		background-position:0px -32px !important;
	}
	#thread div.best_answer_background .recipe_comment { 
		border: 2px solid #755531 !important;
	}
";
...
function best_answer_init() {		
	...
	if ((!empty($bb_current_user->ID) && $bb_current_user->ID==$topic->topic_poster)) {$best_answer['can_edit']=true;}
...
function best_answer() {
	...
	if ($topic->topic_posts>1 && $bb_post->post_position>1 && (empty($best_answer['forums']) || isset($best_answer['forums'][$topic->forum_id]))) {	
		if (!$best_answer['can_edit']) { 
			...
			if (isset($best_answer[$topic->topic_id][$bb_post->post_id])) {
				echo "<a title='Nuimti gero atsakymo statusą' href='$url' class='best_answer best_answer_selected'>".$best_answer['text']."</a>";
			} elseif ($best_answer['max']==1 || $best_answer['count']<$best_answer['max']) {
				echo "<a title='Pažymėti kaip gerą atsakymą' href='$url' class='best_answer'>".$best_answer['text']."?</a>";
			} 
		}
	}
}
