...
class Entities_Gapi
{
  ...
  {
    $response = $this->httpRequest(Entities_Gapi::account_data_url, array('start-index'=>$start_index,'max-results'=>$max_results), null, $this->generateAuthHeader());
  ...
  public function requestReportData($report_id, $dimensions, $metrics, $sort_metric=null, $filter=null, $start_date=null, $end_date=null, $start_index=1, $max_results=30)
  {
    ...
    $parameters['prettyprint'] = Entities_Gapi::dev_mode ? 'true' : 'false';
    
    $response = $this->httpRequest(Entities_Gapi::report_data_url, $parameters, null, $this->generateAuthHeader());
  ...
  protected function authenticateUser($email, $password)
  {
    $post_variables = array(
      ...
      'Email' => $email,
      'Passwd' => $password,
      'source' => Entities_Gapi::interface_name,
      'service' => 'analytics'
    );
    
    $response = $this->httpRequest(Entities_Gapi::client_login_url,null,$post_variables);
  ...
  {
    $interface = Entities_Gapi::http_interface;
    
    if(Entities_Gapi::http_interface =='auto')
    {
    ...
    {
      throw new Exception('Invalid http interface defined. No such interface "' . Entities_Gapi::http_interface . '"');
    }
  }
  ...
  public function __call($name,$parameters)
  {
    ...
    $parameter_key = Entities_Gapi::array_key_exists_nc($name,$this->report_root_parameters);
    ...
    $aggregate_metric_key = Entities_Gapi::array_key_exists_nc($name,$this->report_aggregate_metrics);
...
class gapiAccountEntry
{
  ...
  public function __call($name,$parameters)
  {
    ...
    $property_key = Entities_Gapi::array_key_exists_nc($name,$this->properties);
...
class gapiReportEntry
{
  ...
  public function __call($name,$parameters)
  {
    ...
    $metric_key = Entities_Gapi::array_key_exists_nc($name,$this->metrics);
    ...
    $dimension_key = Entities_Gapi::array_key_exists_nc($name,$this->dimensions);
