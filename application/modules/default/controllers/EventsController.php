...
class EventsController extends Zend_Controller_Action {
	...
	function indexAction() {
		$select = $this->db->select()
				...
				->group("beer_events.event_id")
				->order("beer_events.event_start DESC");
		if (isset($this->user->user_id) && !empty($this->user->user_id) && $this->user->user_id != 0){
			if ($this->user->user_type != "admin") {
				$select->where("beer_events.event_published = ?", '1');
			}
		} else {
			$select->where("beer_events.event_published = ?", '1');
		}
	...
	function deleteAction() {
		...
		if ($this->canEdit($group_id)) {
			$db->delete("beer_events", "event_id = '" . $event_id . "'");
		}
		$this->_redirect("/ivykiai");
	}
							...
							if (isset($this->user->user_id)) {
								...
								if ($this->canEdit($group_id)) {
									$this->db->update("beer_events", array(
										"event_name" => htmlentities(stripslashes(strip_tags($_POST['event_name'])), ENT_COMPAT, "UTF-8"),
